        ...
        <div id="tagsWrapper" class="flex md:flex-col h-full relative scroll-shadow-s after:transition-colors after:duration-500">
            <div id="tagsContainer"
                ...
                <div onclick="toggleTagParams(<%= tag.getIdTag() %>)"
                    class="min-h-[26px] text-xs px-2 py-1 dark:bg-[<%= tag.getHex() %>]/20 bg-[<%= tag.getHex() %>]/20 flex w-fit rounded-md md:w-full md:justify-center md:max-w-full md:text-base md:py-1 md:font-semibold cursor-pointer select-none outline-[<%= tag.getHex() %>] <%= request.getParameter("tag") != null && request.getParameter("tag").equals(String.valueOf(tag.getIdTag())) ? "outline outline-[2.5px]" : "" %>">
                    <p class="whitespace-nowrap text-[<%= tag.getHex() %>]"><%= tag.getLibelle() %></p>
                </div>
                <% } %>

                <%-- Add and edit tags --%>
                <div class="flex gap-2 items-center w-fit md:w-full md:mt-2">
                    <div onclick="showToastCreateTag()"
                        class="min-h-[26px] md:min-h-[34px] text-xs px-2 py-1 dark:bg-darkNote bg-white flex  rounded-md md:w-full justify-center md:py-1 md:font-semibold cursor-pointer select-none border border-green-500 transition-colors duration-500">
                    ...
                    <div onclick="showToastEditTag()"
                        class="min-h-[26px] md:min-h-[34px] text-xs px-2 py-1 dark:bg-darkNote bg-white flex w-fit rounded-md md:w-full justify-center md:py-1 md:font-semibold cursor-pointer select-none border border-orange-500 transition-colors duration-500">
                        <p class="flex items-center">
                    ...
                    <div onclick="showToastDeleteTag()"
                        class="min-h-[26px] md:min-h-[34px] text-xs px-2 py-1 dark:bg-darkNote bg-white flex w-fit rounded-md md:w-full justify-center md:py-1 md:font-semibold cursor-pointer select-none border border-red-500 transition-colors duration-500">
                        <p class="flex items-center">
    ...
    Toast.fire({
        ...
        html: `
            ...
            <div class="flex flex-col gap-1">
                ...
                <input type="text" id="NewTagName" name="NewTagName" class="border border-gray-300 rounded-md p-2 mt-2 focus:outline-none" placeholder="Nouveau nom" autocomplete="off">
                <div class="flex items-center gap-2">
                    <div class="text-sm">Couleur :</div>
        ...
        if (result.isConfirmed) {
            if(document.getElementById("NewTagName").value.length === 0 || document.getElementById("NewTagName").value === ""){
                Toast.fire({
                    icon: 'error',
                    timer: 2000,
                    title: 'Erreur',
...
function showToastDeleteTag(){
    ...
    Toast.fire({
        ...
        html: `
            ...
            <div class="flex flex-col gap-1">
                <div class="text-xs text-red-500">Attention, toutes les notes associées à cette étiquette se retrouveront sans étiquette</div>
