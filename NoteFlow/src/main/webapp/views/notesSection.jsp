<div class="w-full flex p-4 overflow-y-auto overflow-x-hidden justify-center">
        ...
        <div class="flex flex-wrap w-full gap-4 xl:gap-8 justify-center 2xl:justify-start content-start">
           <% for(ExtendedNote note : notes){  %>
				...
				<div class="flex flex-col relative w-full md:w-[49.9%] h-[220px] bg-white shadow rounded-md px-4 hover:cursor-pointer hover:scale-[1.005] transition-all hover:shadow-xl hover:border-[1px] hover:border-gray-200 flex-1-1">
				<a href="index.jsp?note=<%= note.getIdNote() %>">
					<span class="absolute w-[4px] h-[45px] bg-[<%= note.getHex() %>] left-0 top-[15px]"></span>
					<div class="flex w-full h-[40px] items-center" ><p class="font-semibold max-w-full overflow-hidden text-ellipsis line-clamp-1 text-base md:text-xl"><%= note.getTitle() %></p><span class="w-4 h-4 min-w-4 min-h-4 rounded-full flex ms-2 bg-[<%= note.getHex() %>] <%= note.getTitle().length() >= 38 ? "hidden" : "hidden md:flex"  %>"></span></div>
					...
					<div class="flex w-full h-[50px] items-center justify-end ">
				...
				<span class="flex gap-2 z-50 absolute bottom-5">
						...
						<form method="POST" id="form-delete-<%= note.getIdNote() %>">
							<svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="black" class="hover:fill-red-500 bi bi-trash3-fill" viewBox="0 0 16 16" onclick="showToastDelete(<%= note.getIdNote() %>, '<%= note.getTitle() %>')">
